...
1. 读 [README](README.md)、[产品大纲](product/outline.md)、相关的[功能设计](product/features/README.md)和[待办](iterations/backlog.md)。
...
- **产品大纲**（`product/outline.md`）是第一层：只写定位、用户、原则、每个支柱与模块的基础描述和当前进度、分期；不写功能细节。
- **功能设计**（`product/features/`，按[写法规范](product/features/README.md#写法规范)）是第二层：每个模块一份，开头的「功能项与进度」表是功能 ID、阶段与状态的唯一来源，正文定义用户操作、规则、数据、实现约束与验收。状态只能凭真实证据推进；状态变化后同步大纲中该模块的进度描述。可见行为、数据归属、权限及交付范围的改变，先改设计并获得用户确认。
