...
Git 已初始化为 main，当前没有提交；作者姓名和邮箱待用户提供。不能把未提交文件称为 Git 版本。

写入本轮方案前创建了包含初始化骨架和当时 .git 的完整归档：

- 路径：custom-projects/.backups/dsh-product-before-prd-20260911.tar.gz
- SHA-256：28b803074aad5aba554d41e301f44731c55c3082e36a99d47331a5c4c6e7ffcd

恢复时先校验并解压到新的空目录，比较后选择文件，不直接覆盖当前项目。这是临时本地恢复措施，不代替 Git 历史或异地备份。

## GitHub 上传时机

用户于 2026-09-11 明确要求：v0.1 完成前仅在本地维护，之后再上传 GitHub。因此当前不创建远端、不配置连接、不推送；GitHub 不是本地开发或验收的前提。取得本地提交身份后即可保存 Git 提交，无需 GitHub 账号。

v0.1 完成后仍由用户决定上传时间，只有收到明确指令才处理远端。当前任务、方案和验收记录都保存在本仓库文档中。
